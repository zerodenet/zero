...
pub struct MieruManagedUdpFlowResume {
    server: String,
    port: u16,
    protocol: mieru::udp::MieruUdpFlowResume,
}
...
pub struct MieruManagedUdpFlowConfig<'a> {
    server: &'a str,
    port: u16,
    protocol: mieru::udp::MieruUdpFlowConfig<'a>,
}

pub type MieruManagedStreamUdpResume = ManagedTupleUdpResume<MieruManagedUdpFlowResume>;

impl ProtocolManagedStreamUdpResumeMetadata for MieruManagedStreamUdpResume {
...
impl<'a> MieruManagedUdpFlowConfig<'a> {
    pub fn new(server: &'a str, port: u16, username: &'a str, password: &'a str) -> Self {
        Self {
            server,
            port,
            protocol: mieru::udp::MieruUdpFlowConfig::new(username, password),
        }
    }
...
impl MieruManagedUdpFlowResume {
    fn new(server: &str, port: u16, protocol: mieru::udp::MieruUdpFlowResume) -> Self {
        Self {
    ...
    fn connector_flow(
        &self,
        session_id: u64,
    ) -> ManagedConnectorFlow<mieru::udp::MieruUdpConnectorFlow> {
        ManagedConnectorFlow(mieru::udp::connector_flow_from_resume(
            &self.protocol,
            &self.server,
            self.port,
            session_id,
    ...
    ) -> Result<mieru::udp::MieruUdpFlowConnection, EngineError>
    where
        ...
        OpenSocketFut: Future<Output = Result<TokioSocket, EngineError>> + Send,
    {
        ...
        mieru::udp::establish_udp_flow_with_resume(stream, &self.protocol)
            .await
            .map_err(|error| {
...
impl ManagedConnectorFlowOps for mieru::udp::MieruUdpConnectorFlow {
...
impl ProtocolManagedTupleUdpFlowResumeConnectionOps for MieruManagedUdpFlowResume {
    type ConnectorFlow = ManagedConnectorFlow<mieru::udp::MieruUdpConnectorFlow>;
    type RawConnection = mieru::udp::MieruUdpFlowConnection;
...
#[async_trait::async_trait]
impl ManagedTupleUdpConnectionOps for mieru::udp::MieruUdpFlowConnection {
    ...
    async fn send_protocol_packet(
        ...
        mieru::udp::MieruUdpFlowConnection::send(self, target, port, payload).await
    }

    fn subscribe_protocol_packets(&self) -> mieru::udp::MieruUdpFlowResponseReceiver {
