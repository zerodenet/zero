...
async fn relays_tcp_through_vless_wss_chained_outbound() {
    ...
    let upstream_config = RuntimeConfig::parse(&format!(
        r#"{{
            "inbounds": [
                {{
                    "tag": "upstream-vless-wss-in",
                    "listen": {{ "address": "127.0.0.1", "port": {upstream_port} }},
                    "protocol": {{
                        "type": "vless",
                        "users": [{{ "id": "{USER_ID}", "principal_key": "node:upstream-wss" }}],
                        "tls": {{
                            "cert_path": "{}",
                            "key_path": "{}"
                        }},
    ...
    let outer_config = RuntimeConfig::parse(&format!(
        r#"{{
            ...
            "route": {{
                "rules": [],
                "final": {{ "type": "route", "outbound": "vless-wss-chain" }}
            }}
        }}"#,
        escape_json_path(&tls.cert_path),
    ))
    ...
    client.write_all(b"wss1").await.expect("write payload");
    let mut echoed = [0_u8; 4];
    client.read_exact(&mut echoed).await.expect("read payload");
    assert_eq!(&echoed, b"wss1");
