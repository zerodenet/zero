...
async fn relays_tcp_through_vless_grpc_chained_outbound() {
    let echo_port = free_port();
    let upstream_port = free_port();
    let outer_port = free_port();
    let tls = test_tls_material();
    let service_name = "/zero.vless.grpc/Tun";
    ...
    let upstream_config = RuntimeConfig::parse(&format!(
        r#"{{
            "inbounds": [
                {{
                    "tag": "upstream-vless-grpc-in",
                    "listen": {{ "address": "127.0.0.1", "port": {upstream_port} }},
                    "protocol": {{
                        "type": "vless",
                        "users": [{{ "id": "{USER_ID}", "principal_key": "node:upstream-grpc" }}],
                        "tls": {{
                            "cert_path": "{}",
                            "key_path": "{}"
                        }},
