...
async fn relays_udp_through_socks5_to_shadowsocks_relay_chain() {
    ...
    let first_packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"ssr1")
    ...
    assert_eq!(response.payload, b"ssr1");

    let second_packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"ssr2")
    ...
    assert_eq!(response.target, Address::Ipv4([127, 0, 0, 1]));
    assert_eq!(response.port, echo_port);
    assert_eq!(response.payload, b"ssr2");
