...
async fn relays_udp_through_vless_outbound() {
            ...
            "inbounds": [
                {{
                    ...
                    "protocol": {{
                        "type": "vless",
                        "users": [
                            {{ "id": "{USER_ID}", "principal_key": "node:udp-upstream" }}
                        ]
                    }}
                }}
            ],
    ...
    let outer_config = RuntimeConfig::parse(&format!(
        r#"{{
            ...
            "route": {{
                "rules": [],
                "final": {{ "type": "route", "outbound": "vless-udp-chain" }}
            }}
        }}"#
    ))
    ...
    let packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"vudp")
    ...
    wait_for("outer udp session to record vless outbound", || {
        outer_probe
            .active_sessions()
            .first()
            .map(|session| {
                session.network == zero_core::Network::Udp
                    && session.outbound_tag.as_deref() == Some("vless-udp-chain")
