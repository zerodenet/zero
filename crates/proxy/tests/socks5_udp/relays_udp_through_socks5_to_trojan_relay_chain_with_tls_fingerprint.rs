...
/// Regression coverage for the trojan **relay-stream** TLS client fingerprint
/// path: when a trojan outbound is the non-final hop of a relay group, its UDP
/// stream is established via `establish_over_relay_stream` →
/// `zero_transport::tls::connect_tls_stream`, which must honour
/// `client_fingerprint` (custom TLS 1.3 ClientHello) instead of silently
/// falling back to rustls defaults.
#[tokio::test]
#[cfg(all(feature = "socks5", feature = "trojan"))]
async fn relays_udp_through_socks5_to_trojan_relay_chain_with_tls_fingerprint() {
    ...
    let packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"tf1")
    ...
    assert_eq!(response.payload, b"tf1");

    let packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"tf2")
    ...
    assert_eq!(response.target, Address::Ipv4([127, 0, 0, 1]));
    assert_eq!(response.port, echo_port);
    assert_eq!(response.payload, b"tf2");
