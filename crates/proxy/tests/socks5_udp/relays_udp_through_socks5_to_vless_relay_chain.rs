...
async fn relays_udp_through_socks5_to_vless_relay_chain() {
    ...
    let packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"relay")
    ...
    assert_eq!(response.payload, b"relay");

    let packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"again")
    ...
    assert_eq!(response.target, Address::Ipv4([127, 0, 0, 1]));
    assert_eq!(response.port, echo_port);
    assert_eq!(response.payload, b"again");

    wait_for("outer udp session to record relay chain outbound", || {
        outer_probe
            .active_sessions()
            .first()
            .map(|session| {
    ...
    drop(control);
    wait_for("outer udp relay chain session to complete", || {
        outer_probe
            .completed_sessions()
            .first()
            .map(|session| {
