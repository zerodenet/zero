...
async fn relays_udp_through_shadowsocks_outbound() {
    ...
    let packet = build_udp_packet(&Address::Ipv4([127, 0, 0, 1]), echo_port, b"ssup")
    ...
    assert_eq!(response.payload, b"ssup");

    wait_for("outer udp session to record shadowsocks outbound", || {
        outer_probe
            .active_sessions()
            .first()
            .map(|session| {
                session.network == zero_core::Network::Udp
                    && session.outbound_tag.as_deref() == Some("ss-udp-chain")
    ...
    drop(control);
    wait_for("outer udp shadowsocks session to complete", || {
        outer_probe
            .completed_sessions()
            .first()
            .map(|session| {
