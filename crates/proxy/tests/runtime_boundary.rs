...
fn stream_udp_managers_do_not_rebuild_protocol_cache_keys() {
    ...
    assert!(
        !mieru_managed.contains("MieruUdpCacheKey::relay")
            ...
            && mieru_connector.contains(".connector_flow(endpoint.server, endpoint.port, session_id)")
            && trojan_connector.contains(".connector_flow(endpoint.server, endpoint.port, session_id)")
...
fn trojan_udp_flow_resume_is_protocol_owned() {
    ...
    assert!(
        !managed.contains("resume.flow_cache_key(")
            ...
            && connector.contains(".connector_flow(endpoint.server, endpoint.port, session_id)")
...
fn adapters_do_not_own_udp_packet_path_cache_key_formats() {
    ...
    assert!(
        !socks5_adapter.contains("socks5::udp_cache_key")
            ...
            && socks5_packet_path.contains("spec.carrier_descriptor")
