...
fn vless_udp_transport_opening_lives_in_transport_crate() {
    let runtime = read("src/adapters/vless/udp/manager.rs");
    let model = read("src/adapters/vless/udp/manager/model.rs");
...
fn vmess_udp_transport_opening_lives_in_transport_crate() {
    let runtime = read("src/adapters/vmess/udp/manager.rs");
    let model = read("src/adapters/vmess/udp/manager/model.rs");
...
fn h2_udp_stream_pump_uses_protocol_flow_resume_boundary() {
    ...
    for forbidden in [
        ...
        assert!(
            !stream.contains(forbidden),
            "Hysteria2 UDP stream glue should delegate packet construction/encoding and pump detail to protocols/hysteria2; found `{forbidden}`"
        );
    }
    assert!(
...
fn vmess_inbound_udp_response_encoding_stays_in_protocol_crate() {
    ...
    assert!(
        !helper.contains("VmessInboundUdpPayload")
            ...
            && mux.contains("vmess::VmessInboundUdpCodec.write_response_tokio")
            && mux.contains("vmess::VmessInboundUdpCodec.send_mux_response")
...
fn protocol_udp_cached_flow_fast_path_lives_outside_state_root() {
    ...
    let protocol_stream_sender =
        manifest_dir().join("src/runtime/udp_flow/protocol_state/stream_sender.rs");
    ...
    for forbidden in [
        ...
        assert!(
            !state.contains(forbidden),
            "src/runtime/udp_flow/protocol_state/mod.rs should keep managed stream forwarding details in managed/stream_sender.rs; found `{forbidden}`"
        );
    }
    assert!(
        stream_sender.exists() && !old_cached.exists() && !protocol_stream_sender.exists(),
        "managed stream UDP sender forwarding should live under managed/stream_sender.rs, not protocol_state"
    );
    assert!(
...
fn h2_udp_datagram_codec_lives_outside_manager() {
    ...
    assert!(
        !manager_model.contains("struct H2Entry")
            ...
            && !stream.contains("use zero_core::UdpFlowPacket")
...
fn h2_udp_packet_stream_tasks_live_outside_manager() {
    ...
    assert!(
        stream_path.exists(),
        "Hysteria2 UDP packet stream glue should live in h2_manager/stream.rs"
    );
    for forbidden in [
