...
#[async_trait]
pub(crate) trait DatagramUdpResponder<S>: Send
where
    S: Send,
{
        ...
        if poll_upstream {
            ...
            select! {
                read = responder.read_inbound_dispatch(&source) => {
                    if !process_datagram_read(proxy, &mut dispatch, &mut responder, read).await {
                        break;
                    }
                }
    ...
    R: DatagramUdpResponder<S>,
{
    ...
    match dispatch_inbound_udp_packet(proxy, dispatch, &inbound_dispatch, responder.auth()).await {
