...
- UDP packet-path carrier descriptors, datagram sources, lookup keys, flow snapshots, and packet-path chain orchestration live under `runtime::udp_flow`; protocol-specific UDP flow snapshots live under `protocol_runtime::udp`. Generic runtime flow state uses neutral variants (`Direct`, `Relay`, `Datagram`, `StreamPacket`, `PacketPathDatagram`) plus opaque protocol or packet-path snapshots and must not declare protocol-named outbound variants or carrier enums.
- Runtime UDP flow facades may create tracked `UdpFlowOutbound` values, but protocol snapshot construction belongs to `protocol_runtime::udp::flow_snapshot`; runtime code must call snapshot constructors rather than constructing `ProtocolUdpFlowSnapshot::{Socks5,Shadowsocks,Hysteria2,Trojan,Mieru}` variants directly.
