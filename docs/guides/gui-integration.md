...
| 结构化校验诊断 | `error.details[]` 数组扩展 | 已实现 — `error.details[]`（每个 `{field_path?, message}`）由配置校验填充，含字段级路径（如 `inbounds[0]`、`runtime.udp_upstream_idle_timeout_seconds`）；`error.field_path` 保留 | 在表单具体字段旁展示错误 |
| 路由解释 | `diagnostics.trace_route` 扩展返回命中规则链 | 已实现 — 返回 target / port / protocol / effective_mode / route_action / `matched_rule`（命中规则的 0 基 `index` + `condition` 摘要；命中 final 动作时为 null） | 做”为什么走这个节点”的调试面板 |
| DNS 调试 | DNS 缓存查询、Fake IP 查询命令 | 部分实现 — `diagnostics.dns_lookup` 已提供基础 DNS 解析，缓存和 Fake IP 待补齐 | 排查透明代理和分流问题 |
| 日志流 | 受过滤的日志订阅或查询 | 未实现 | GUI 内置实时日志页，减少用户查文件 |
| flow 关闭原因 | completed flow 增加标准 close reason | 未实现 | 连接列表可区分正常结束、主动关闭、超时、上游失败 |
| policy 探测详情 | probe 事件补充成员选择原因 | 已实现 — `policy.probe.completed` 事件包含成员级 healthy / latency_ms / error（成员选择原因待补齐） | 节点页可展示延迟和失败原因 |
