...
| PushConnector（Panel Connector） | 窄统计提供器、`CommandService` | 周期心跳、节点汇总统计和远程命令接收 | 否 |
...
命令执行结果通过日志记录，不在心跳中回传。`config.validate` 和 `config.apply` 刻意不通过 PushConnector 暴露；完整配置替换应走独立部署流程。未来可扩展 `POST /api/v1/nodes/{node_id}/commands/{cmd_id}/result`。
