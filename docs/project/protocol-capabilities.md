...
- `shadowsocks_2022_tcp_outbound_not_externally_validated`: Shadowsocks AEAD 2022 (SIP022) 全部路径已实现并通过内置测试：TCP 请求/响应头部、SIP022 3.1.5 服务端重放 salt 池、AEAD 2022 UDP 服务端响应（回填客户端 session id）。TCP 入站方向已通过 `shadowsocks-rust` 参考客户端 (`sslocal`) 端到端互操作性验证；AEAD 2022 UDP 服务端响应已通过手动探针（`protocols/shadowsocks/examples/udp_server_probe.rs`，DNS 往返）验证。仅 TCP 出站方向尚未在外部参考实现上完成端到端验证（此环境下的 `ssserver` 单次读取存在 Windows 环境缺陷，已通过参考对对照测试排除 Zero 自身缺陷）。
