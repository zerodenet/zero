...
- `shadowsocks_2022_tcp_replay_protection_lacks_salt_pool`: Shadowsocks AEAD 2022 TCP 已实现 SIP022 请求/响应头部协议（含 30 秒时间戳窗口校验、响应头请求 salt 回填校验、变量头 padding 校验），但服务端尚未实现 60 秒请求 salt 池的完整重放保护。
...
| `shadowsocks` | 普通 AEAD TCP 和 UDP datagram 路径完备，包括 Shadowsocks UDP over SOCKS5、大 TCP 载荷分块、错误密码拒绝、数据包路径中继链以及针对 `shadowsocks-rust` 的本地外部 UDP 出站互操作性（覆盖所有支持的 cipher）；AEAD 2022 (SIP022) TCP 已实现完整请求/响应头部协议（固定+变量头、时间戳校验、请求 salt 回填校验、padding），覆盖三个 blake3 cipher 双向基线路径 | AEAD 2022 服务器端 UDP 外部互操作性、60 秒请求 salt 池完整重放保护和针对 `shadowsocks-rust` 的外部 TCP 端到端互操作性验证不完整 |
