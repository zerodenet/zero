...
Workspace dependency direction is enforced independently of proxy layout. `zero-transport` owns reusable carrier implementations, neutral carrier/transport helpers, and shared execution contracts, but it does not own protocol-named transport modules and it does not import or project `zero-engine::ResolvedLeafOutbound`. Engine-leaf-to-protocol-profile projection is protocol integration work and lives in the owning `zero-proxy::adapters` module behind the proxy-local `ProtocolTransportLeafResolver` boundary. Protocol crates may expose optional runtime features that depend on `zero-transport` and `zero-platform-tokio`, but they must not depend on config, engine, proxy, or router union types.

`zero-config` owns closed configuration ADTs, required/range checks, reference validation, and cross-field transport compatibility. It does not recreate protocol-private parsers: VLESS/VMess UUIDs, VMess cipher selection, VLESS Reality/XHTTP values, Shadowsocks cipher/2022 key material, and SOCKS5 credential wire limits delegate to validation-only protocol APIs. Validation-only protocol features must not compile data-plane modules.

`zero-engine::runtime` is a facade over explicit state domains. Configuration reload/persistence and subscriber notification live in `runtime/configuration.rs`; selector/urltest state and policy events live in `runtime/policy.rs`; stats/event/sink control-plane projection lives in `runtime/observability.rs`. The runtime root retains Engine construction and core routing/session coordination rather than regrowing those domains inline.
...
| `socks5` | handshake, UDP association framing/classification, protocol-owned transport request/profile shaping | generic TCP/UDP carrier primitives | listener/association handoff and upstream handler registration; stateless | support, inbound, TCP outbound, UDP flow, packet path, upstream provider |
| `shadowsocks` | cipher/session/framing, UDP codec, protocol-owned transport request/profile shaping | generic carrier opening and shared wrappers | listener plus managed datagram handler assembly; stateless | support, inbound, TCP outbound, UDP flow, packet path, managed provider |
| `hysteria2` | authenticated QUIC/session, protocol-owned bind/profile shaping, and UDP semantics | generic QUIC carrier primitives | listener plus managed handler assembly; stateless | support, inbound, TCP outbound, UDP flow, managed provider |
| `mieru` | tunnel negotiation, session/framing, UDP plan, protocol-owned transport request/profile shaping | generic carrier execution helpers | listener and managed-flow delegation; stateless | support, inbound, TCP outbound, UDP flow, managed provider |
| `vless` | handshake, accepted routes, carrier selection, inbound/outbound transport plans, MUX/UDP plan and pool | generic TLS/WS/gRPC/H2/XHTTP/QUIC carriers and neutral bridge contracts | collapsed capability root owning leaf projection plus transport bridge handoff | support, inbound, TCP outbound, UDP flow, packet path, managed provider |
| `vmess` | handshake, crypto/framing, accepted routes, carrier selection, inbound/outbound transport plans, MUX/UDP plan and pool | generic TLS/WS/gRPC/H2 carriers and neutral bridge contracts | collapsed capability root owning leaf projection plus transport bridge handoff | support, inbound, TCP outbound, UDP flow, managed provider |
| `trojan` | handshake, TLS profile resolution, accepted routes, carrier selection, inbound/outbound transport plans, and UDP plan | generic TLS/WS/gRPC/H2 carriers and neutral bridge contracts | collapsed capability root owning leaf projection plus transport bridge handoff | support, inbound, TCP outbound, UDP flow, managed provider |
...
The `runtime::udp_dispatch` root facade re-exports only generic dispatch result types (`FlowFailure`, `FlowStartResult`, `UdpCandidate`). Protocol-named UDP flow request models stay in protocol-owned transport helpers instead of being collected at the root facade. The `src/runtime/udp_dispatch` and `src/runtime/udp_flow/managed` families keep only neutral tracked-start, registration, relay-forward, and two-stream orchestration glue; for VLESS/VMess/Trojan managed stream flows, owning proxy adapters perform the one-time `ResolvedLeafOutbound` projection and then hand protocol-owned prepared transport/flow state into those generic helpers. Proxy `udp.rs`, `udp/managed.rs`, and `udp/transport.rs` shells for those protocols must stay absent. `runtime::udp_dispatch` keeps only the narrow context needed to invoke registered handlers and managed-flow helpers without exposing protocol state.
...
VLESS outbound deferred-response handshake selection and response-stream wrapping stay in `protocols/vless::outbound`; carrier selection, direct/relay transport opening, QUIC selection, inbound transport-plan preparation, fallback replay classification, and shared VLESS bridge metadata now live in `protocols/vless::transport`. Proxy adapters must not directly drive `DeferredTcpTunnelProtocol` or construct `DeferredVlessResponseStream`.
The same ownership rule now covers VLESS inbound base transport setup: TLS ClientHello peeking, TLS accept, Reality upgrade, carrier dispatch, owned inbound transport-plan construction, TCP accepted-route/fallback replay wrappers, stream-route fallback adapters, listener-side recorded mux defaults, and the QUIC bind plan live in `protocols/vless::transport`; `src/adapters/vless.rs` owns capability forwarding and leaf projection; `src/adapters/vless/listener.rs` builds the protocol-owned request, calls the shared logged TCP/QUIC listener loops directly, and hands accepted routes into recorded post-accept runtime helpers; and shared fallback replay relay plus `src/runtime/inbound_route.rs` stay protocol-neutral and post-accept only.
...
VLESS inbound XHTTP / HTTPUpgrade / WebSocket / gRPC / H2 carrier selection follows the same split in `protocols/vless::transport`; the proxy-side `src/adapters/vless.rs` bridge keeps only route-object to runtime TCP/UDP/MUX handoff, while fallback replay relay stays in shared runtime glue.
...
VMess inbound WebSocket and gRPC carrier framing are also transport-owned (`crates/transport/src/ws.rs`, `crates/transport/src/grpc.rs`); protocol accept-route dispatch lives in `src/adapters/vmess.rs`, `protocols/vmess/src/transport.rs` owns carrier accept/build helpers, owned inbound transport-plan construction, and route-handoff dispatch helpers, and proxy listener glue keeps only runtime TCP/UDP/MUX dispatch handoff.
VMess inbound raw / WebSocket / gRPC carrier selection is centralized in `protocols/vmess/src/transport.rs`; proxy listener glue does not keep per-carrier helper functions or local mux-default bundles, and Trojan inbound route dispatch follows the same pattern through protocol-owned route-handoff helpers in `protocols/trojan/src/transport.rs`.
Inbound bridge layout follows responsibility count rather than a uniform directory template. HTTP, Mixed, Shadowsocks, Mieru, and Hysteria2 keep bind/spawn delegation, listener lifecycle, and post-accept runtime handoff in a single `src/adapters/<protocol>/inbound.rs`; an `inbound/` directory must not exist solely to contain `listener.rs`. SOCKS5 retains `src/adapters/socks5/inbound/` because listener lifecycle and client UDP association are separate sibling responsibilities. Sibling top-level UDP adapter modules continue to own outbound UDP-specific handoff. In the current tree these external protocols no longer keep proxy-local `inbound/request.rs`; protocol-owned transport helpers now live under `protocols/{socks5,shadowsocks,hysteria2,mieru,vless,vmess,trojan}/src/transport.rs`, while `zero-transport` stays limited to carrier primitives, generic transport helpers, and neutral bridge contracts. Proxy `inbound/request.rs` and `inbound/dispatch.rs` should stay absent once no local wrapper remains; adapter-local listener bridges build protocol-owned request objects, call the logged listener loops directly, and hand accepted routes into post-accept runtime helpers; and `src/runtime/inbound_route.rs` stays post-accept only.
...
VLESS/VMess outbound MUX pool cache state and concurrency reuse live in `protocols/vless::mux_pool` and `protocols/vmess::mux`. Protocol-owned transport bridge objects in `protocols/{vless,vmess,trojan}::transport` hold any protocol-specific pool state, prepared-request materialization, resume metadata, and reload eviction; proxy adapter roots call those bridge helpers directly; `src/runtime/udp_flow/managed/bridge.rs` owns neutral two-stream startup such as the VLESS paired relay path; and `src/adapters/identity.rs` must not regrow shared outbound wrappers. Shared transport opener glue and cached MUX connection wiring live in `crates/transport/src/transport_plan.rs` and `crates/transport/src/mux_stack.rs`, while `zero-transport` stays responsible only for carrier accept/build helpers, transport-neutral primitives, and TLS/WS/gRPC/H2/QUIC/REALITY carrier setup. Trojan transport no longer rebuilds `ClientTlsConfig` from protocol fields; it consumes protocol-owned TLS profiles through `zero-traits::ClientTlsProfile`. Adapter roots stay at strict capability forwarding and hold only the protocol-owned transport bridge object rather than protocol-specific pool fields; proxy `tcp.rs`, `udp.rs`, and `udp/managed.rs` bridge shells for VLESS/VMess/Trojan must stay absent. For inbound protocols that need protocol-private transport setup, proxy `src/adapters/<protocol>/inbound.rs` continues to own carrier-specific bind/spawn delegation when a local wrapper is still needed; VLESS/VMess/Trojan keep only the thin capability hook in `src/adapters/{vless,vmess,trojan}.rs`; adapter-local `listener.rs` bridges pass protocol-owned request objects directly into shared logged listener loops plus post-accept runtime helpers.
...
Adding an inbound-capable protocol therefore means adding protocol-owned inbound/UDP responder APIs, transport-owned acceptor/profile construction where needed, a thin adapter preparation bridge, a capability implementation, and registration in `register.rs`. The adapter does not run a listener loop or spawn tasks. `runtime::inbound_operation` executes the prepared TCP/QUIC listener operation and hands accepted protocol routes into `runtime::inbound_route::{dispatch_protocol_stream_route, dispatch_protocol_mux_route}` or the shared UDP relay glue.
...
Adapter subdirectories require multiple real sibling responsibilities. Mieru keeps its managed stream-packet flow in `src/adapters/mieru/udp.rs` because it has no sibling packet-path or association module. Hysteria2 and Shadowsocks retain their UDP responsibility splits, while SOCKS5 retains packet-path and upstream-association preparation. VLESS, VMess, and Trojan retain adapter-local `listener.rs` because their capability roots and transport listener preparation are separate responsibilities. Transport-owned plans normalize protocol endpoint and resume state into `ManagedDatagramStartPlan`, `ManagedStreamPacketBridgePlan`, or owned transport leaves. Runtime-owned generic operations execute those plans; adapter code must not retain `Proxy`, spawn tasks, run listener loops, or introduce protocol-named executor types under `runtime`.

TCP and UDP transport bridges follow prepare/execute separation. Adapters select their bridge and synchronously prepare owned transport state. `runtime::tcp_dispatch::operation` and `runtime::udp_dispatch::operation` execute generic bridge operations and must not mention VLESS, VMess, Trojan, SOCKS5, Shadowsocks, Mieru, or Hysteria2 concrete types. Packet-path and registered-association factories receive only `UdpRuntimeServices`, which exposes narrow socket, peer-resolution, traffic, and lifecycle accounting operations without exposing `Proxy`.

Boundary tests lock these responsibilities and dependency directions rather than exact file paths. Refolding a single-purpose adapter module or moving a facade file is allowed when the adapter/runtime/protocol/transport ownership rules remain true.
