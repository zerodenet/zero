...
External-protocol inbound bind/spawn entrypoints live on the owning adapter bridge surface.
Simpler protocols may still use `crates/proxy/src/adapters/<protocol>/inbound.rs`, while VLESS/VMess/Trojan keep their thin runtime handoff collapsed directly in `crates/proxy/src/adapters/{vless,vmess,trojan}.rs`. Adapters own the protocol-facing bind/spawn bridge surface; `Proxy` does not own protocol `run_*_listener_with_bound` methods.
...
- `adapters/mod.rs` only declares concrete adapter modules and re-exports adapter types that genuinely live under adapters. Registry construction and protocol dispatch stay outside this facade; for VLESS/VMess/Trojan, managed UDP stream handler registration now comes through the adapter roots themselves because proxy `udp.rs` shells stay absent and the runtime-facing implementation is transported through bridge objects plus the generic helper surface in `adapters/common.rs`.
- `inbound/mod.rs` only declares proxy-owned inbound listener modules (`direct`, `system`, `tun`). External protocol request models plus bind/spawn/listener/session glue stay under the owning adapter bridge surface: `src/adapters/<protocol>/inbound*` when a local wrapper still exists, otherwise directly in the adapter root for VLESS/VMess/Trojan.
...
Runtime UDP flow facades may create tracked `UdpFlowOutbound` values, but they only register protocol resume state with `ProtocolUdpState` and store the returned opaque managed-flow reference. Runtime flow bookkeeping does not store or import `ProtocolUdpFlowSnapshot`.
The `runtime::udp_dispatch` root facade re-exports only generic dispatch result types (`FlowFailure`, `FlowStartResult`, `UdpCandidate`). Protocol-named UDP flow request models stay in proxy transport bridges or `protocols/*` helpers instead of being collected at the root facade. For VLESS/VMess/Trojan managed stream flows, `zero-transport` owns `ResolvedLeafOutbound` projection, transport-leaf endpoint facts, one-time protocol plan materialization, carrier opening, shared bridge state, managed-flow connection/connector traits, UDP bridge-local stage / expected-leaf metadata, and managed-stream handler stage metadata. On the proxy side, `src/adapters/common.rs` keeps only neutral relay-prefix orchestration plus generic validation/runtime wrappers, while the adapter roots `src/adapters/{vless,vmess,trojan}.rs` keep only capability forwarding and direct calls into those generic helpers / transport bridge objects. Proxy `udp.rs`, `udp/managed.rs`, and `udp/transport.rs` shells for those protocols must stay absent. `runtime::udp_dispatch` keeps only the narrow managed-flow/helper bridge needed to pass chain task context into `ProtocolUdpState`.
Existing protocol UDP flow forwarding keeps `runtime::udp_flow::protocol_state` as a dispatch facade. Protocol-specific snapshot extraction, upstream request shaping, packet I/O, and manager send request construction live in protocol-owned adapter modules or `protocols/*`, not in generic runtime facades.
...
Per-protocol outbound TCP glue lives in the owning adapter capability bridge, but for VLESS/VMess/Trojan the adapter roots forward `ResolvedLeafOutbound` directly into transport-owned bridge objects instead of reviving proxy `tcp.rs` shells or extracting leaf/request details locally. Protocol handshake/session details live in `protocols/*`. Transport-leaf endpoint facts, TCP open-result normalization, and bridge-local stage / expected-leaf metadata are transport-owned traits implemented in `zero-transport`, not in proxy adapter modules. `src/outbound/<protocol>.rs` helper modules, `src/adapters/<protocol>/tcp/connect/*` forwarding shells, and VLESS/VMess/Trojan proxy `tcp.rs` bridge shells must not be reintroduced. Runtime modules dispatch through `ProtocolInventory` and `TcpOutboundCapability`.
...
VLESS direct outbound transport opening, including QUIC selection, also lives in `crates/transport/src/vless_transport.rs`; on the proxy side, `zero-transport` now owns `ResolvedLeafOutbound` projection plus one-time materialization of leaf-local protocol request / flow / MUX plans, transport-profile mapping, transport-leaf endpoint facts, TCP open-result normalization, and the transport bridge objects for VLESS/VMess/Trojan, while `src/adapters/common.rs` keeps only generic adapter-local error/runtime wrappers plus neutral shared relay-prefix orchestration over those transport-owned leaves and the adapter roots consume those bridge objects directly for TCP/UDP capability dispatch.
The same ownership rule now covers VLESS inbound base transport setup: TLS ClientHello peeking, fallback-ALPN replay classification, TLS accept, Reality upgrade, carrier dispatch, owned inbound transport-plan construction, TCP accepted-route/fallback replay wrappers, stream-route fallback adapters, and the QUIC bind plan all live in `crates/transport/src/vless_transport.rs`; `src/adapters/vless.rs` owns capability forwarding, bind/spawn glue, and route-object handoff into shared runtime helpers. Shared fallback replay relay and `src/runtime/inbound_route.rs` stay protocol-neutral.
...
VMess inbound raw / WebSocket / gRPC carrier selection is centralized in `crates/transport/src/vmess_transport.rs`; proxy listener glue does not keep per-carrier helper functions, and Trojan inbound route dispatch follows the same pattern through transport-owned route-handoff helpers in `src/adapters/trojan.rs`.
SOCKS5, Shadowsocks, Mieru, and Hysteria2 inbound bridge layout now follows the same adapter-local split even when the carrier is simpler: `src/adapters/<protocol>/inbound.rs` keeps only bind/spawn glue, `inbound/request.rs` keeps protocol-owned acceptor/profile request models when still needed, and sibling `udp*.rs` or other local bridge modules keep carrier-to-runtime handoff. Hysteria2 QUIC cert/key bind-plan ownership lives in `crates/transport/src/hysteria2_quic.rs` instead of the adapter bind hook. For VLESS/VMess/Trojan, the owned inbound request objects now live in `zero-transport`; proxy `inbound/request.rs` and `inbound/dispatch.rs` should stay absent once no local wrapper remains, with thin runtime handoff kept directly inside the adapter root.
Neutral TCP / QUIC accept-loop lifecycle, per-connection task fan-out, and connection-error logging live in `src/runtime/listener_loop.rs`. Protocol listener modules choose only the protocol dispatch function and request object they pass into those shared helpers; they do not duplicate `run_tcp_listener_loop` / `run_quic_stream_listener_loop` wrappers once a shared logged helper exists.
Relay-chain TCP hops follow the same split: adapters reuse transport-over-stream builders in `zero-transport` for VLESS/VMess/Trojan and then invoke protocol-owned handshake/session APIs over the wrapped stream.
VLESS/VMess outbound MUX pool cache state and concurrency reuse live in `protocols/vless::mux_pool` and `protocols/vmess::mux`. `VlessStreamBridge` / `VmessStreamBridge` in `zero-transport` now hold the shared pool state plus reload eviction; proxy adapter roots call those transport bridge helpers directly; `src/adapters/common.rs` keeps only generic outbound error/runtime wrappers plus neutral shared relay-prefix orchestration such as VLESS two-stream startup. Shared transport opener glue and cached MUX connection wiring live in `crates/transport/src/transport_plan.rs` and `crates/transport/src/mux_stack.rs`, while `crates/transport/src/{vless,vmess}_transport.rs` still own the protocol-specific leaf projection, prepared-request materialization, and MUX carrier build/use details. `protocols/vless` and `protocols/vmess` now own prepared outbound request bundles that already capture the protocol-selected MUX transport profile; `zero-transport` may pass only neutral `zero-traits::StreamMuxTransportHints` into those prepared bundles while owning outbound leaf projection, split-http relay decisions, transport-leaf endpoint facts, TCP open-result normalization, and the owned inbound request objects for VLESS/VMess/Trojan. Trojan transport no longer rebuilds `ClientTlsConfig` from protocol fields; it consumes protocol-owned owned TLS profiles through `zero-traits::ClientTlsProfile`. Adapter roots stay at strict capability forwarding and hold only the transport bridge object rather than protocol-specific pool fields; proxy `tcp.rs`, `udp.rs`, and `udp/managed.rs` bridge shells for VLESS/VMess/Trojan must stay absent. For inbound protocols that need protocol-private transport setup, proxy `src/adapters/<protocol>/inbound.rs` continues to own accept-route dispatch and route-object/runtime handoff when a local wrapper is still needed, while VLESS/VMess/Trojan now keep that thin bridge directly in `src/adapters/{vless,vmess,trojan}.rs`; `zero-transport` owns carrier accept/build helpers, transport-neutral primitives, owned inbound request/transport-plan construction, and TLS/WS/gRPC/H2/QUIC/REALITY carrier setup.
