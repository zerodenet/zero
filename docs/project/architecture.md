...
The `runtime::udp_dispatch` root facade re-exports only generic dispatch result types (`FlowFailure`, `FlowStartResult`, `UdpCandidate`). Protocol-named UDP flow request models stay in their protocol flow submodules instead of being collected at the root facade. Protocol-named UDP flow facade modules, including VLESS/VMess, live under `protocol_runtime::udp::*_flow`; `runtime::udp_dispatch` keeps only the narrow managed-flow/helper bridge needed to pass chain task context into `ProtocolUdpState`.
Existing protocol UDP flow forwarding keeps `protocol_runtime::udp::state::forward.rs` as a dispatch facade. Protocol-specific snapshot field extraction, upstream request shaping, and manager send request construction live in `state/forward/<protocol>.rs`, not in the forward facade.
...
`protocol_runtime::udp` root does not re-export packet-path helper functions or generic packet-path runtime types. Adapters call `runtime::udp_flow::packet_path` constructors and `runtime::udp_flow::packet_path_chain::carriers::*` when bridging capability methods.
