...
Workspace dependency direction is enforced independently of proxy layout. `zero-transport` owns reusable carrier implementations plus concrete protocol transport plans, leaves, and bridge operations, but it does not import or project `zero-engine::ResolvedLeafOutbound`. Engine-leaf-to-transport-leaf projection is protocol integration work and lives in the owning `zero-proxy::adapters` module behind the proxy-local `ProtocolTransportLeafResolver` boundary. Protocol crates remain runtime-neutral and do not depend on config, engine, platform, proxy, router, or transport crates.
