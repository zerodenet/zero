...
这份文档描述请求从外部进入 Zero 内核，到完成出站转发和观测结算的实际流转。

这里的“生命周期”指内核处理链路，不指版本规划或产品线节奏。
...
- `policies`：选择或组合策略，例如 `selector`、`fallback`、`urltest`
- `target`：路由和模式引用的统一目标

当前实现里的 `outbound_groups` 是过渡命名，不应固化为长期 API 概念。
...
当前 `/status`、`/runtime`、`/config` 是最小本地导出，不是长期 API 定稿。
...
不要把当前内部实现名 `Session` 或过渡配置名 `outbound_groups` 固化成正式控制面概念。
