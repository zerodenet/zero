...
10. `urltest` 组可以通过配置解析和校验
11. `urltest` 能在后台探测后更新当前选中的成员
12. `global mode -> urltest` 可用
13. `urltest` 选中的成员可用于 `TCP`
14. `urltest` 选中的成员可用于 `SOCKS5 UDP ASSOCIATE`
15. 状态导出能看到 `urltest` 当前选择和最近一次探测结果
...
- 至少提供一份 `fallback` 示例和一份 `urltest` 示例
