...
QUIC 数据包路径载体已支持：`Hysteria2PacketPath` 实现 `UdpPacketPath`，经 QUIC 连接承载内部 datagram（如 Shadowsocks），`[Hysteria2, Shadowsocks]` UDP 中继链可用（见 e2e 测试 `relays_udp_through_hysteria2_to_shadowsocks_packet_path_chain`）。
