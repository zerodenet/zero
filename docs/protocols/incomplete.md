...
本页集中记录协议层未完成能力。它用于 GUI、面板和开发者判断哪些能力可以直接对接，哪些需要等待后续实现。

## Shadowsocks

| 缺口 | 影响 | 完成标准 |
|------|------|----------|
| `shadowsocks_2022_tcp_header_is_not_implemented` | AEAD 2022 TCP 不能宣称外部完整互通 | 实现 SIP022 TCP request/response header，并通过 `shadowsocks-rust` TCP 互通 |
...
| `relay_stream_tls_client_fingerprint_is_not_supported` | relay stream 上的 TLS final hop 不能完全控制 fingerprint | relay-stream TLS 握手获得等价 raw socket 控制能力，或定义明确不可支持边界 |

## Trojan

| 缺口 | 影响 | 完成标准 |
|------|------|----------|
| 外部互通覆盖不足 | 当前不能宣称生产级完整兼容 | 使用基线实现进行 TCP 和 UDP 外部互通测试 |
...
| 缺口 | 影响 | 完成标准 |
|------|------|----------|
| 外部互通覆盖不足 | 不能宣称生产级完整兼容 | 使用基线 Hysteria2 实现进行 TCP stream 和 UDP datagram 互通 |
...
| 缺口 | 影响 | 完成标准 |
|------|------|----------|
| 外部互通覆盖不足 | 不能宣称生产级完整兼容 | 使用基线 Mieru 客户端/服务端互通测试 |
| inbound TCP 仍是实验描述 | GUI 不应默认作为稳定 server 能力展示 | descriptor、测试和文档同时提升为稳定能力 |

## VMess

| 缺口 | 影响 | 完成标准 |
|------|------|----------|
| `vmess_udp_is_not_implemented` | 无 VMess UDP | 实现 VMess UDP wire format 和 runtime dispatch |
| `cipher_auto_is_not_supported` | Xray/Clash 常见导出不能直接使用 | 支持或明确转换 `cipher: auto` |
| 外部 AEAD 兼容不足 | 不能作为生产 VMess 节点能力 | 与 Xray VMess AEAD 节点互通 |
| MUX 不支持 | 无 VMess MUX | 实现 MUX 或保持 `unsupported` |

## 通用要求

协议从 `partial` 提升到 `supported` 需要同时满足：
